...
namespace PAP
{
    public partial class frmAluguer_Ténis : Form
    {        
        string[] strArray;
        string[] strArray1;
        int qtd1;
        ...
        {            
            try
            {
               ...
               // SqlConnection connect1 = new SqlConnection(Properties.Resources.Connect1);
                ...
                cmd.CommandText = "Select id_Campo from Campo where Nome='"+cbbcampo.SelectedItem.ToString()+"'and Nome_Desporto='Ténis'";
            ...
            {                         
                ...
                if (cbbqtdpessoas.SelectedItem == "2")
                {
                    if (txtNome.Text == txtNome1.Text)
                        return false;
                    if ((reader.GetDateTime(0) == Convert.ToDateTime(dtpData.Value) && reader.GetString(5) == txtNome.Text && reader.GetString(6) == txtNome1.Text &&// nao pode tar em ccampos diferentes a mesma hora o mesmo utente
                        (((HFim * 60) + (MFim)) == ((nupHFinal.Value * 60) + (nupMFinal.Value)) && ((Hinic * 60) + (Minic)) == ((nupHinicial.Value * 60) + (nupMinicial.Value))) && reader.GetInt32(1) != result)
                ...
                if (cbbqtdpessoas.SelectedItem == "3")
                 {
                    if (txtNome.Text == txtNome1.Text || txtNome.Text == txtNome2.Text || txtNome1.Text == txtNome2.Text)
                        return false;
                        if ( (reader.GetDateTime(0) == Convert.ToDateTime(dtpData.Value) && reader.GetString(5) == txtNome.Text && reader.GetString(6) == txtNome1.Text && reader.GetString(7) == txtNome2.Text &&// nao pode tar em ccampos diferentes a mesma hora o mesmo utente
                        (((HFim * 60) + (MFim)) == ((nupHFinal.Value * 60) + (nupMFinal.Value)) && ((Hinic * 60) + (Minic)) == ((nupHinicial.Value * 60) + (nupMinicial.Value))) && reader.GetInt32(1) != result)
                ...
                if (cbbqtdpessoas.SelectedItem == "4")
                {
                    ...
                    if (txtNome.Text == txtNome1.Text || txtNome.Text == txtNome2.Text || txtNome.Text == txtNome3.Text || txtNome1.Text == txtNome2.Text || txtNome1.Text == txtNome3.Text || txtNome2.Text == txtNome3.Text)
                        return false;
                    if (txtNome.Text == txtNome1.Text || txtNome.Text == txtNome2.Text || txtNome1.Text == txtNome2.Text)
                        if (reader.GetString(2) == "Ténis" && (reader.GetDateTime(0) == Convert.ToDateTime(dtpData.Value) && reader.GetString(5) == txtNome.Text && reader.GetString(6) == txtNome1.Text && reader.GetString(7) == txtNome2.Text && reader.GetString(8) == txtNome3.Text && // nao pode tar em ccampos diferentes a mesma hora o mesmo utente
                        (((HFim * 60) + (MFim)) == ((nupHFinal.Value * 60) + (nupMFinal.Value)) && ((Hinic * 60) + (Minic)) == ((nupHinicial.Value * 60) + (nupMinicial.Value))) && reader.GetInt32(1) != result)
                    ...
                    teste();
                        if (check2() == true)
                        {
                            if (check() == true)
                            {
                                ...
                                cmd.Parameters.AddWithValue("@Nome_desporto", "Ténis");
                                ...
                                cmd.ExecuteNonQuery();
                                MessageBox.Show("Aluguer de Ténis adicionado à BD");
                                connect.Close();
                                Consumo();
                                Limpeza();
                                False();
        ...
        private void frmAlguer_Ténis_Load(object sender, EventArgs e)
        {
           // False();
        }
        ...
        private void Preço()
        {
            ...
            connect.Open();
            SqlCommand cmd = connect.CreateCommand();
            cmd.CommandText = "Select preço from Preço where id_desporto='Ténis' and tipo_consumo='Aluguer'";
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
            ...
            if (cbbqtdpessoas.SelectedItem == "1")
            {
                try
                {
                    if (Socio(txtNome.Text) == true)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Desporto", "Ténis");
                        cmd.Parameters.AddWithValue("@id_campo", result);
        ...
        private void frmAlguer_Ténis_FormClosing(object sender, FormClosingEventArgs e)
        {
